...
points:            .word 0    #Guarda la cantidad de manzanas consumidas
str_msg:           .asciiz "Puntaje: "  # String a imprimir
framerate:         .word 120  #ms para el sleep del loop de entrada
.text
...
entradaLoop:
	...
	lw $t4, framerate   #Carga el framerate en t4
	addi $v0, $zero, 32	# syscall sleep
	add $a0, $zero, $t4	# 120 ms
	syscall
...
exitVel:
	...
	lw $t1, points                       #carga los puntos
	addi $t1, $t1, 1                     #suma un punto por la manzana consumida
	sw $t1, points                       #guardar puntos en memoria
	lw $t4, framerate                    #Carga el framerate en t4
	subi $t4, $t4, 2                     #Resta 2 al framerate por cada manzana consumida
	sw   $t4, framerate                  #Guarda el framerate actualizado en memoria
...
noEstaEnManzana:
	...
	##Imprimir puntaje
	la $a0, str_msg       # Cargar la direcci�n del string en $a0
    	li $v0, 4             # Cargar el c�digo de la llamada al sistema para imprimir un string
    	syscall               # Realizar la llamada al sistema
    	
    	lw $a0, points        # Cargar los puntos en $a0
    	li $v0, 1             # Cargar el c�digo de la llamada al sistema para imprimir un entero
    	syscall               # Realizar la llamada al sistema
